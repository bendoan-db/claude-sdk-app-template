...
export const databricksFetch: typeof fetch = async (input, init) => {
  ...
  const backgroundMode = headers.get('x-background-mode') === 'true';
  ...
  headers.delete('x-background-mode');
  ...
  // Inject background mode for long-running agent tasks
  if (backgroundMode && requestInit?.body && typeof requestInit.body === 'string') {
    try {
      const body = JSON.parse(requestInit.body);
      requestInit = {
        ...requestInit,
