...
| `agent_server/start_server.py` | FastAPI entrypoint — loads `.env`, registers DB lifecycle hooks, starts uvicorn |
| `agent_server/long_running_server.py` | `LongRunningAgentServer` — background mode, `GET /responses/{id}`, timeout protection |
...
- **Lakebase database**: instance `doan-agent-memory`, database `databricks_postgres` — `CAN_CONNECT_AND_CREATE` permission
...
- `LAKEBASE_INSTANCE_NAME` ← from the `database` resource (may be a hostname, resolved at startup)
